			...
			<ul>
				...
				<?php if (is_array($result)): ?>
					<?php foreach ($result as $key1 => $value): ?>
						...
						<?php if ($value['top']):?>
								<span style = "width:50px; height:20px; overflow:hidden; display:inline-block;">
								已通过</span>
								<a style = "width:70px; height:20px; overflow:hidden; display:inline-block;" href = "<?php echo $value['url_cancel'];?>">取消通过</a>
						<?php else:?>
								<span style = "width:50px; height:20px; overflow:hidden; display:inline-block;">
								未通过</span>
								<a style = "width:70px; height:20px; overflow:hidden; display:inline-block;" href = "<?php echo $value['url_check'];?>">审核通过</a>
						<?php endif;?>
